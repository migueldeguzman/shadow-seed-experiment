...
LOG_DIR="/Users/miguelitodeguzman/ailab/lab-protocol/experiments/rsi-009/data"
...
echo "=== RSI-009 Session: $SESSION_NAME ==="
echo "Time: $TIMESTAMP"
echo "Model: $MODEL"
...
for SUBJECT in "${SUBJECTS[@]}"; do
  SUBJECT_SHORT="${SUBJECT#lab-rsi009-}"
  LOG_FILE="${LOG_DIR}/${SUBJECT_SHORT}-${SESSION_NAME}-$(date +%Y%m%dT%H%M%S).log"
  
  echo "▶ Triggering $SUBJECT_SHORT..."
  
  # Run in background, capture output
  # No ANTHROPIC_API_KEY unsetting needed — it's not set in the environment
  docker exec "$SUBJECT" claude -p \
    --model "$MODEL" \
