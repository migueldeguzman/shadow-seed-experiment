#!/bin/bash
# =============================================================
# RSI-010 Session Trigger — Docker + Ollama/Qwen3
# Runs sessions inside Docker containers (matching RSI-008/009
# isolation methodology). Connects to Ollama on host.
#
# Usage: ./trigger-session.sh [session_name]
# Example: ./trigger-session.sh hourly
#
# Fixes (2026-03-02): Added lock file to prevent stacking,
# timeouts on all curl/docker calls.
#
...
LOG_DIR="/Users/miguelitodeguzman/ailab/lab-protocol/experiments/rsi-010/data"
LOCK_FILE="/tmp/rsi-010-trigger.lock"
...
echo "=== RSI-010 Session: $SESSION_NAME ==="
echo "Time: $TIMESTAMP"
echo "Model: qwen3-coder-next (via Ollama → host.docker.internal)"
...
# ── Check containers ────────────────────────────────────────
RUNNING=$(docker ps --filter "name=lab-rsi010" --format "{{.Names}}" 2>/dev/null | wc -l | tr -d ' ')
...
WARMUP_RESPONSE=$(curl -s -m "$WARMUP_TIMEOUT" "http://localhost:11434/api/generate" \
  -d '{"model":"qwen3-coder-next","prompt":"hello","stream":false,"options":{"num_predict":1}}' 2>&1)
