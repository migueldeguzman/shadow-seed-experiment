#!/usr/bin/env python3
"""
RSI-010 Website Updater — Pulls live data + file contents from
Docker containers, updates the RSI-010 page, builds, and pushes.
...
PAGE = "/Users/miguelitodeguzman/Projects/individuationlab/website/src/pages/rsi-010/index.astro"
...
DATA_DIR = "/Users/miguelitodeguzman/ailab/lab-protocol/experiments/rsi-010/data"
...
def get_subject_data(subject):
    container = f"lab-rsi010-{subject}"
    code = r"""
import os, json
...
def count_sessions():
    ...
    with open(TRIGGER_LOG) as f:
        content = f.read()
    count = content.count("All 8 subjects complete")
    times = re.findall(r"All 8 subjects complete.*?\nTime: (.+)", content)
...
def update_page(live_html, total_sessions, last_time):
    ...
    status_text = f"{total_sessions} sessions completed · Last: {last_time} · Qwen3-Coder-Next 80B (Q4_K_M)"
    new_content = re.sub(
...
def build_and_push():
    ...
    if result.returncode != 0:
        ...
        subprocess.run(
            ["git", "commit", "-m", f"RSI-010: Live data update — {total} sessions (auto)"],
